<template>
  <div class="min-h-screen" style="background-color: var(--color-gray-50);">
    <!-- Header -->
      ...
      <div class="max-w-7xl mx-auto px-6">
        <div class="flex justify-between items-center h-20">
          <div class="flex items-center">
            <div>
              <h1 class="text-2xl font-semibold" style="color: var(--color-gray-900);">{{
                t('invoiceGenerator') }}</h1>
              <p class="text-sm font-medium" style="color: var(--color-gray-500);">{{
                t('createProfessionalInvoices') }}</p>
            </div>
          </div>
          <div class="flex items-center space-x-4">
            <div class="relative">
              <button @click="showThemeSelector = !showThemeSelector"
                ...
                style="border-color: var(--color-gray-200); color: var(--color-gray-700); --tw-ring-color: var(--color-gray-900);">
                <div class="w-3 h-3 rounded-full"
                  :style="{ backgroundColor: PDF_THEMES.find(t => t.id === selectedTheme)?.primary }">
                </div>
                <span>{{PDF_THEMES.find(t => t.id ===
              ...
              <div v-if="showThemeSelector"
                ...
                <div class="p-2">
                  <div v-for="theme in PDF_THEMES" :key="theme.id"
                    @click="selectedTheme = theme.id; showThemeSelector = false"
                    ...
                    <div class="w-4 h-4 rounded-full" :style="{ backgroundColor: theme.primary }">
                    </div>
                    <div class="flex-1">
                      <div class="font-medium text-sm" style="color: var(--color-gray-900);">
                        {{ theme.name }}</div>
                      <div class="text-xs" style="color: var(--color-gray-500);">
                        {{ theme.description }}
                      </div>
                    </div>
                    <svg v-if="selectedTheme === theme.id" class="w-4 h-4" style="color: var(--color-gray-900);"
            ...
            <button @click="showCustomers = true"
              ...
              style="border-color: var(--color-gray-200); color: var(--color-gray-600);"
              @mouseenter="$event.target.style.backgroundColor = 'var(--color-gray-100)'"
            ...
            <button @click="saveCurrentInvoice" :disabled="!canDownload"
              ...
              style="border-color: var(--color-gray-200); color: var(--color-gray-600);"
              @mouseenter="!canDownload || ($event.target.style.backgroundColor = 'var(--color-gray-100)')"
              @mouseleave="$event.target.style.backgroundColor = 'transparent'">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="relative">
              <button @click="showExportOptions = !showExportOptions" :disabled="!canDownload || isGeneratingPDF"
                ...
                @mouseenter="$event.target.style.backgroundColor = 'var(--color-gray-800)'"
                @mouseleave="$event.target.style.backgroundColor = 'var(--color-gray-900)'">
                <div v-if="isGeneratingPDF" class="absolute inset-0 flex items-center justify-center"
                ...
                <span>{{ isGeneratingPDF ? 'Generating...' : 'Export'
                  }}</span>
                <svg v-if="!isGeneratingPDF" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
              ...
              <div v-if="showExportOptions && !isGeneratingPDF"
    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-8 lg:gap-12">
        <!-- Form Section -->
        <div class="bg-white rounded-2xl border p-8" style="border-color: var(--color-gray-200);">
          <div class="space-y-12">
            <!-- Company Logo -->
            <div class="space-y-6">
              <div class="flex items-center space-x-3">
                <div class="w-1.5 h-1.5 rounded-full" style="background-color: var(--color-gray-900);">
                </div>
                <h2 class="text-xl font-semibold" style="color: var(--color-gray-900);">Company
                  Logo</h2>
              </div>
              <div class="space-y-4">
                <div v-if="!invoice.logo"
                  ...
                  <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"
                    ...
                    <svg class="w-8 h-8" style="color: var(--color-gray-400);" fill="none" stroke="currentColor"
                      viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  ...
                  <p class="font-semibold text-sm mb-2" style="color: var(--color-gray-600);">
                    Upload your company logo</p>
                  <p class="text-xs mb-4" style="color: var(--color-gray-400);">
                    PNG, JPG up to 2MB (auto-compressed for smaller PDFs)</p>
                  <label for="logo-upload"
                    class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-medium cursor-pointer transition-all duration-200"
                    style="background-color: var(--color-gray-900); color: white;">
                    Choose File
                  </label>
                  <input id="logo-upload" type="file" accept="image/*" @change="handleLogoUpload" class="hidden">
                </div>
                <div v-else class="border rounded-2xl p-6 flex items-center space-x-4"
                  style="border-color: var(--color-gray-200);">
                  <img :src="invoice.logo" alt="Company Logo" class="w-16 h-16 object-contain rounded-lg">
                  <div class="flex-1">
                    <p class="font-semibold text-sm" style="color: var(--color-gray-900);">
                      Default Logo</p>
                    <p class="text-xs" style="color: var(--color-gray-500);">
                      Will appear on all new invoices
                    </p>
                  </div>
                  <button @click="removeLogo" class="p-2 rounded-lg transition-colors duration-200"
                    style="color: var(--color-gray-400);" @mouseenter="$event.target.style.color = '#ef4444'"
            ...
            <div class="space-y-6">
              <div class="flex items-center space-x-3">
                <div class="w-1.5 h-1.5 rounded-full" style="background-color: var(--color-gray-600);">
                </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">{{
                    t('invoiceNumber') }}</label>
                  <input v-model="invoice.number" type="text"
                    ...
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
                    placeholder="INV-001">
                </div>
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">{{
            ...
            <div class="space-y-6">
              <div class="flex items-center space-x-3">
                ...
                <h2 class="text-xl font-semibold" style="color: var(--color-gray-900);">From</h2>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Business
                    Name</label>
                  <input v-model="invoice.from.businessName" type="text"
                    class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
                    placeholder="Your Business">
                </div>
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Tax
                    ID</label>
                  <input v-model="invoice.from.taxId" type="text"
                    class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
                    placeholder="Tax ID">
                </div>
              ...
              <div class="space-y-3">
                <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Address</label>
                <input v-model="invoice.from.address" type="text"
                  class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                  style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Email</label>
                  <input v-model="invoice.from.email" type="email"
                    class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
                    placeholder="your@email.com">
                </div>
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Phone</label>
                  <input v-model="invoice.from.phone" type="tel"
                    class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
            ...
            <div class="space-y-6">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                  ...
                  <h2 class="text-xl font-semibold" style="color: var(--color-gray-900);">To
                  </h2>
                </div>
                <div class="flex items-center space-x-2">
                  ...
                  <button @click="saveCurrentCustomer" :disabled="!invoice.to.customerName.trim()"
                    ...
                    style="border-color: var(--color-gray-200); color: var(--color-gray-600);"
                    @mouseenter="!invoice.to.customerName.trim() || ($event.target.style.backgroundColor = 'var(--color-gray-100)')"
                    @mouseleave="$event.target.style.backgroundColor = 'transparent'">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3-3m0 0l-3 3m3-3v12">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Customer
                    Name</label>
                  <input v-model="invoice.to.customerName" type="text"
                    class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
                ...
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Tax
                    ID</label>
                  <input v-model="invoice.to.taxId" type="text"
                    class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
                    placeholder="Tax ID">
                </div>
              ...
              <div class="space-y-3">
                <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Address</label>
                <input v-model="invoice.to.address" type="text"
                  class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                  style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
                  placeholder="Street, City, State, ZIP">
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Email</label>
                  <input v-model="invoice.to.email" type="email"
                    class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
                ...
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Phone</label>
                  <input v-model="invoice.to.phone" type="tel"
                    class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"
            ...
            <div class="space-y-6">
              ...
              <div v-if="invoice.items.length === 0" class="rounded-2xl border-2 border-dashed p-16 text-center"
                style="border-color: var(--color-gray-200); background-color: var(--color-gray-50);">
                <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"
                  style="background-color: var(--color-gray-200);">
                  <svg class="w-8 h-8" style="color: var(--color-gray-400);" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                ...
                <p class="font-semibold text-lg mb-2" style="color: var(--color-gray-600);">No items
                  yet</p>
                <p class="text-sm" style="color: var(--color-gray-400);">Add your
                  first item to get started</p>
              </div>
              ...
              <div v-for="(item, index) in invoice.items" :key="item.id"
                ...
                style="border-color: var(--color-gray-200); background-color: var(--color-gray-50);">
                <div class="flex items-center justify-between">
                  <span class="text-sm font-semibold" style="color: var(--color-gray-600);">Item
                    {{ index + 1 }}</span>
                  <button @click="removeItem(index)" class="transition-colors duration-200 p-2 rounded-lg"
                    style="color: var(--color-gray-400);" @mouseenter="$event.target.style.color = '#ef4444'"
                    @mouseleave="$event.target.style.color = 'var(--color-gray-400)'">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="space-y-4">
                  <div class="space-y-3">
                    <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Description</label>
                  ...
                  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="space-y-3">
                      <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Quantity</label>
                      <input v-model.number="item.quantity" type="number" placeholder="1" min="0" step="0.01"
                        class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                        style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);">
                    </div>
                    <div class="space-y-3">
                      <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Price</label>
                      <input v-model.number="item.price" type="number" placeholder="0.00" min="0" step="0.01"
                        class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                        style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);">
                    </div>
                    <div class="space-y-3">
                      <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Tax
                        %</label>
                      <input v-model.number="item.tax" type="number" placeholder="0" min="0" max="100" step="0.01"
                        class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                        style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);">
                    </div>
                    <div class="space-y-3">
                      <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Total</label>
                      <div class="h-12 rounded-xl flex items-center px-4 text-sm font-bold"
                        style="background-color: var(--color-gray-100); color: var(--color-gray-900);">
                        ${{
            ...
            <div class="space-y-6">
              <div class="flex items-center space-x-3">
                ...
                <h2 class="text-xl font-semibold" style="color: var(--color-gray-900);">Payment
                  Information</h2>
              </div>
              <div class="space-y-4">
                <div class="space-y-3">
                  <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Payment
                    Instructions</label>
                  <textarea v-model="invoice.payment.instructions" rows="3"
                    ...
                    style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"></textarea>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="space-y-3">
                    <label class="block text-sm font-medium" style="color: var(--color-gray-600);">Bank
                      Details</label>
                    <textarea v-model="invoice.payment.bankDetails" rows="3"
                      placeholder="Bank name, account number, routing..."
                      class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200 resize-none"
                      style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);"></textarea>
                  </div>
                  <div class="space-y-3">
                    <label class="block text-sm font-medium" style="color: var(--color-gray-600);">PayPal
                      Email</label>
                    <input v-model="invoice.payment.paypalEmail" type="email" placeholder="paypal@business.com"
                      class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                      style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);">
                    <label class="block text-sm font-medium mt-3" style="color: var(--color-gray-600);">Crypto
                      Address</label>
                    <input v-model="invoice.payment.cryptoAddress" type="text" placeholder="Bitcoin/Ethereum address..."
                      class="w-full border rounded-xl px-4 py-3 text-sm bg-white font-medium focus:outline-none focus:ring-2 transition-all duration-200"
                      style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);">
                  </div>
                </div>
        ...
        <!-- Preview Section -->
        <div class="bg-white rounded-2xl border" style="border-color: var(--color-gray-200);">
          <div id="invoice-preview" class="p-8 bg-white min-h-[700px] overflow-hidden"
            ...
            <div class="space-y-12">
              ...
              <div class="flex justify-between items-start">
                <div class="flex items-start space-x-6">
                  <div v-if="invoice.logo" class="flex-shrink-0">
                    <img :src="invoice.logo" alt="Company Logo" class="w-20 h-20 object-contain">
                  </div>
                  <div>
                    <h1 class="text-5xl font-bold mb-2" style="color: #000;">INVOICE
                    </h1>
                    <p class="text-lg font-medium" style="color: #666;">{{
                ...
                <div class="text-right space-y-3">
                  <div>
                    <div class="text-sm font-semibold text-gray-500 mb-1">
                      DATE</div>
                    <div class="text-lg font-medium" style="color: #000;">{{
              ...
              <div class="grid grid-cols-2 gap-16 py-8">
                <div>
                  <h3 class="text-sm font-semibold text-gray-500 mb-6">
                    FROM</h3>
                  <div class="space-y-2">
                    <div class="text-xl font-semibold" style="color: #000;">{{
                      invoice.from.businessName || '—'
                      }}</div>
                    <div class="text-sm space-y-1" style="color: #666; line-height: 1.6;">
                      <div>{{ invoice.from.address ||
                        '—' }}</div>
                      <div>{{ invoice.from.email ||
                        '—' }}</div>
                      <div>{{ invoice.from.phone ||
                        '—' }}</div>
                      <div v-if="invoice.from.taxId" class="text-xs pt-2" style="color: #999;">Tax
                        ID: {{
                          invoice.from.taxId }}
                      </div>
                    </div>
                ...
                <div>
                  ...
                  <div class="space-y-2">
                    ...
                    <div class="text-sm space-y-1" style="color: #666; line-height: 1.6;">
                      <div>{{ invoice.to.address ||
                        '—' }}</div>
                      <div>{{ invoice.to.email || '—'
                        }}</div>
                      <div>{{ invoice.to.phone || '—'
                        }}</div>
                      <div v-if="invoice.to.taxId" class="text-xs pt-2" style="color: #999;">Tax
                        ID: {{ invoice.to.taxId
                        }}</div>
                    </div>
                  </div>
              ...
              <!-- Items Table -->
              <div class="border-t border-b" style="border-color: #e5e5e5;">
                <table class="w-full">
                  <thead style="background-color: #f8f9fa;">
                    <tr>
                      <th class="px-6 py-4 text-left text-sm font-semibold" style="color: #666;">{{
                        t('tableDescription') }}
                      </th>
                      <th class="px-6 py-4 text-center text-sm font-semibold" style="color: #666;">{{
                        t('tableQty') }}</th>
                      <th class="px-6 py-4 text-right text-sm font-semibold" style="color: #666;">{{
                        t('tablePrice') }}</th>
                      <th class="px-6 py-4 text-right text-sm font-semibold" style="color: #666;">{{
                        t('tableTax') }}</th>
                      <th class="px-6 py-4 text-right text-sm font-semibold" style="color: #666;">{{
                  ...
                  <tbody class="divide-y" style="divide-color: #e5e5e5;">
                    <tr v-if="invoice.items.length === 0">
                      <td colspan="5" class="px-6 py-16 text-center">
                        <div class="text-lg font-medium" style="color: #999;">
                          No items yet
                        </div>
                        <div class="text-sm mt-2" style="color: #ccc;">
                          Add items to see
                          them here</div>
                      </td>
                    </tr>
                    <tr v-for="item in invoice.items" :key="item.id" class="hover:bg-gray-50">
                      <td class="px-6 py-4 text-sm font-medium" style="color: #000;">{{
                        item.description || '—'
                        }}</td>
                      <td class="px-6 py-4 text-center text-sm" style="color: #666;">{{
                        item.quantity || 0 }}
                      </td>
                      <td class="px-6 py-4 text-right text-sm font-mono" style="color: #666;">${{
                        (item.price ||
                        0).toFixed(2) }}</td>
                      <td class="px-6 py-4 text-right text-sm" style="color: #666;">{{
                        (item.tax ||
                        0).toFixed(1) }}%</td>
                      <td class="px-6 py-4 text-right text-sm font-semibold font-mono" style="color: #000;">${{
                        itemTotal(item).toFixed(2)
                        }}</td>
                    </tr>
                  </tbody>
              ...
              <div class="flex justify-end pt-8">
                <div class="w-80 space-y-6">
                  <div class="space-y-4">
                    <div class="flex justify-between items-center">
                      <span class="text-sm font-medium" style="color: #666;">{{
                        t('subtotal') }}</span>
                      <span class="text-sm font-mono" style="color: #000;">${{
                        subtotal.toFixed(2)
                        }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                  ...
                  <div class="border-t pt-6" style="border-color: #000;">
                    <div class="flex justify-between items-center">
                      <span class="text-xl font-bold" style="color: #000;">{{
                        t('totalAmount')
                        }}</span>
                      <span class="text-3xl font-bold font-mono" style="color: #000;">${{
                        total.toFixed(2)
                        }}</span>
                    </div>
                  </div>
              ...
              <!-- Payment Information -->
              <div
                v-if="invoice.payment.instructions || invoice.payment.bankDetails || invoice.payment.paypalEmail || invoice.payment.cryptoAddress || paymentQRCode"
                class="border-t pt-8" style="border-color: #e5e5e5;">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <!-- Payment Details -->
                  <div
                    v-if="invoice.payment.instructions || invoice.payment.bankDetails || invoice.payment.paypalEmail || invoice.payment.cryptoAddress"
                    class="space-y-4">
                    ...
                    <div v-if="invoice.payment.instructions" class="space-y-2">
                      <div class="text-sm font-medium" style="color: #000;">{{
                        t('paymentInstructions')
                        }}</div>
                      <div class="text-sm whitespace-pre-line" style="color: #666;">{{
                        invoice.payment.instructions
                        }}</div>
                    </div>

                    <div v-if="invoice.payment.bankDetails" class="space-y-2">
                      <div class="text-sm font-medium" style="color: #000;">{{
                        t('bankDetails') }}
                      </div>
                      <div class="text-sm whitespace-pre-line" style="color: #666;">{{
                    ...
                    <div v-if="invoice.payment.paypalEmail" class="space-y-2">
                      <div class="text-sm font-medium" style="color: #000;">{{
                        t('paypalEmail') }}
                      </div>
                    ...
                    <div v-if="invoice.payment.cryptoAddress" class="space-y-2">
                      <div class="text-sm font-medium" style="color: #000;">{{
                        t('cryptoAddress') }}
                      </div>
                      <div class="text-sm font-mono" style="color: #666;">{{
                  ...
                  <!-- QR Code -->
                  <div v-if="paymentQRCode" class="flex flex-col items-center md:items-end space-y-4">
                    <div class="text-center md:text-right">
                      <h3 class="text-sm font-semibold text-gray-500 mb-4">
                        {{
                          t('quickPayment').toUpperCase()
                        }}</h3>
                      <div class="inline-block border p-4 rounded-lg" style="border-color: #e5e5e5;">
                        <img :src="paymentQRCode" alt="Payment QR Code" class="w-24 h-24">
                      </div>
    ...
    <div v-if="showHistory" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
        <!-- Modal Header -->
        <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--color-gray-200);">
          <div>
            <h2 class="text-xl font-semibold" style="color: var(--color-gray-900);">
              Invoice History</h2>
            <p class="text-sm" style="color: var(--color-gray-500);">{{
              invoiceHistory.length }} saved invoices</p>
          </div>
          <button @click="showHistory = false" class="p-2 rounded-lg transition-colors duration-200"
            style="color: var(--color-gray-400);" @mouseenter="$event.target.style.color = 'var(--color-gray-600)'"
            @mouseleave="$event.target.style.color = 'var(--color-gray-400)'">
        ...
        <div class="p-6 overflow-y-auto max-h-[60vh]">
          <div v-if="invoiceHistory.length === 0" class="text-center py-12">
            <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"
              style="background-color: var(--color-gray-200);">
              <svg class="w-8 h-8" style="color: var(--color-gray-400);" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
            ...
            <p class="font-semibold text-lg mb-2" style="color: var(--color-gray-600);">No invoices saved yet</p>
            <p class="text-sm" style="color: var(--color-gray-400);">Save your first
              invoice to see it here</p>
          </div>

          <div v-else class="space-y-4">
            <div v-for="savedInvoice in invoiceHistory" :key="savedInvoice.id"
              class="border rounded-2xl p-6 hover:shadow-md transition-all duration-200 cursor-pointer"
              style="border-color: var(--color-gray-200);" @click="loadInvoice(savedInvoice)">
              <div class="flex items-center justify-between">
                <div class="flex-1">
                  <div class="flex items-center space-x-4 mb-3">
                    ...
                    <span class="text-2xl font-bold font-mono" style="color: var(--color-gray-900);">${{
                      savedInvoice.totalAmount.toFixed(2)
                      }}</span>
                  </div>
                  <div class="flex items-center space-x-6 text-sm" style="color: var(--color-gray-600);">
                    <div class="flex items-center space-x-2">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      ...
                      <span>{{ savedInvoice.customerName
                        }}</span>
                    </div>
                    <div class="flex items-center space-x-2">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <span>{{ savedInvoice.invoice.items.length
                        }} items</span>
                    </div>
                    <div class="flex items-center space-x-2">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <span>{{ new
                        Date(savedInvoice.savedAt).toLocaleDateString()
                        }}</span>
                    </div>
                  </div>
                </div>
                <button @click.stop="deleteInvoice(savedInvoice.id)"
                  class="p-2 rounded-lg transition-colors duration-200 ml-4" style="color: var(--color-gray-400);"
    ...
    <div v-if="showCustomers" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
        <!-- Modal Header -->
        <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--color-gray-200);">
          <div>
            <h2 class="text-xl font-semibold" style="color: var(--color-gray-900);">
              Customer Database</h2>
            <p class="text-sm" style="color: var(--color-gray-500);">{{
              customers.length }} saved customers</p>
          </div>
          <button @click="showCustomers = false" class="p-2 rounded-lg transition-colors duration-200"
            style="color: var(--color-gray-400);" @mouseenter="$event.target.style.color = 'var(--color-gray-600)'"
            @mouseleave="$event.target.style.color = 'var(--color-gray-400)'">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
        ...
        <!-- Search Bar -->
        <div class="px-6 py-4 border-b" style="border-color: var(--color-gray-200);">
          <div class="relative">
            <input v-model="customerSearch" type="text" placeholder="Search customers by name, email, or phone..."
              ...
              style="border-color: var(--color-gray-200); color: var(--color-gray-900); --tw-ring-color: var(--color-gray-900);">
            <svg class="w-5 h-5 absolute left-3 top-1/2 transform -translate-y-1/2"
              style="color: var(--color-gray-400);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="p-6 overflow-y-auto max-h-[50vh]">
          <div v-if="filteredCustomers.length === 0 && !customerSearch.trim()" class="text-center py-12">
            <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"
              style="background-color: var(--color-gray-200);">
              <svg class="w-8 h-8" style="color: var(--color-gray-400);" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
            ...
            <p class="font-semibold text-lg mb-2" style="color: var(--color-gray-600);">No customers saved yet</p>
            <p class="text-sm" style="color: var(--color-gray-400);">Save your first
              customer to see them here</p>
          </div>

          <div v-else-if="filteredCustomers.length === 0 && customerSearch.trim()" class="text-center py-12">
            <p class="font-semibold text-lg mb-2" style="color: var(--color-gray-600);">No customers found</p>
            <p class="text-sm" style="color: var(--color-gray-400);">Try adjusting
              your search terms</p>
          </div>
          ...
          <div v-else class="space-y-4">
            <div v-for="customer in filteredCustomers" :key="customer.id"
              class="border rounded-2xl p-6 hover:shadow-md transition-all duration-200 cursor-pointer"
              style="border-color: var(--color-gray-200);" @click="selectCustomer(customer)">
              <div class="flex items-center justify-between">
                <div class="flex-1">
                  <div class="flex items-center space-x-4 mb-3">
                    <h3 class="text-lg font-semibold" style="color: var(--color-gray-900);">
                    ...
                    <span class="text-xs px-2 py-1 rounded-full"
                      ...
                      Last used {{ new
                        Date(customer.lastUsed).toLocaleDateString()
                      }}
                    </span>
                  </div>
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm" style="color: var(--color-gray-600);">
                    ...
                    <div v-if="customer.phone" class="flex items-center space-x-2">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div v-if="customer.address" class="flex items-center space-x-2 md:col-span-2">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
const translations = {
  EN: {
    ...
    fillRequiredFields: 'Please fill in the required fields (Invoice Number, Business Name, and Customer Name) before saving.',
  ...
  ES: {
    ...
    // Alerts and confirmations
    fillRequiredFields: 'Por favor complete los campos requeridos (Número de Factura, Nombre del Negocio y Nombre del Cliente) antes de guardar.',
    confirmClearInvoice: '¿Está seguro de que desea borrar todos los datos? Esta acción no se puede deshacer.',
...
const t = computed(() => (key: string) => {
  return translations[language.value as keyof typeof translations]?.[key as keyof typeof translations.EN] || key
})
...
const canDownload = computed(() => {
  return invoice.value.number && invoice.value.from.businessName && invoice.value.to.customerName
})
...
const saveCurrentCustomer = () => {
  ...
  if (existingCustomer) {
    if (confirm('A customer with this name or email already exists. Update their information?')) {
...
const filteredCustomers = computed(() => {
  if (!customerSearch.value.trim()) {
    return customers.value.sort((a, b) => new Date(b.lastUsed).getTime() - new Date(a.lastUsed).getTime())
  }
...
const downloadPDF = async () => {
  ...
  try {
    ...
    // Get current theme
    const theme = PDF_THEMES.find(t => t.id === selectedTheme.value) || PDF_THEMES[0]

    let yPosition = margin

    // Helper functions with theme support
    const addText = (text: string, x: number, y: number, fontSize: number = 9, style: string = 'normal', align: string = 'left', color: string = theme.text) => {
    ...
    const addLine = (x1: number, y1: number, x2: number, y2: number, lineWidth: number = 0.2, color: string = theme.accent) => {
      pdf.setLineWidth(lineWidth)
      pdf.setDrawColor(color)
      pdf.line(x1, y1, x2, y2)
    }

    const addRect = (x: number, y: number, width: number, height: number, color: string = theme.primary) => {
    ...
    if (selectedTheme.value === 'creative') {
      ...
      if (invoice.value.logo) {
        try {
          ...
          addText(invoice.value.number || 'INV-001', margin + 30, yPosition + 19, 11, 'normal', 'left', theme.accent)
        } catch (error) {
          addText('INVOICE', margin, yPosition + 15, 28, 'bold', 'left', theme.primary)
          addText(invoice.value.number || 'INV-001', margin, yPosition + 22, 11, 'normal', 'left', theme.accent)
        }
      } else {
        addText('INVOICE', margin, yPosition + 15, 28, 'bold', 'left', theme.primary)
        addText(invoice.value.number || 'INV-001', margin, yPosition + 22, 11, 'normal', 'left', theme.accent)
      ...
      addText('DATE', pageWidth - margin, yPosition + 8, 8, 'bold', 'right', theme.primary)
      addText(formatDate(invoice.value.date), pageWidth - margin, yPosition + 14, 10, 'normal', 'right', theme.text)
      addText('DUE DATE', pageWidth - margin, yPosition + 22, 8, 'bold', 'right', theme.primary)
      ...
      if (invoice.value.logo) {
        try {
          pdf.addImage(invoice.value.logo, 'JPEG', margin, yPosition, 20, 20)
          addText('INVOICE', margin + 30, yPosition + 12, 24, 'normal', 'left', theme.primary)
          addText(invoice.value.number || 'INV-001', margin + 30, yPosition + 18, 10, 'normal', 'left', theme.accent)
        } catch (error) {
          addText('INVOICE', margin, yPosition + 12, 24, 'normal', 'left', theme.primary)
          addText(invoice.value.number || 'INV-001', margin, yPosition + 18, 10, 'normal', 'left', theme.accent)
        }
      } else {
        addText('INVOICE', margin, yPosition + 12, 24, 'normal', 'left', theme.primary)
        addText(invoice.value.number || 'INV-001', margin, yPosition + 18, 10, 'normal', 'left', theme.accent)
      ...
      addText('DATE', pageWidth - margin, yPosition + 5, 8, 'normal', 'right', theme.accent)
      addText(formatDate(invoice.value.date), pageWidth - margin, yPosition + 12, 10, 'normal', 'right', theme.text)
      addText('DUE DATE', pageWidth - margin, yPosition + 18, 8, 'normal', 'right', theme.accent)
      addText(formatDate(invoice.value.dueDate), pageWidth - margin, yPosition + 25, 10, 'normal', 'right', theme.text)
    ...
    if (invoice.value.items.length === 0) {
      ...
      invoice.value.items.forEach((item, index) => {
        ...
        addText(`$${(item.price || 0).toFixed(2)}`, priceX, yPosition, 9, 'normal', 'right', theme.text)
        addText(`${(item.tax || 0).toFixed(1)}%`, taxX, yPosition, 9, 'normal', 'center', theme.text)
        addText(`$${itemTotal(item).toFixed(2)}`, totalX, yPosition, 9, 'bold', 'right', theme.primary)
    ...
    addText(`$${subtotal.value.toFixed(2)}`, totalsValueX, yPosition, 9, 'normal', 'right', theme.text)
    yPosition += 7

    addText('Tax', totalsLabelX, yPosition, 9, 'normal', 'left', theme.text)
    addText(`$${totalTax.value.toFixed(2)}`, totalsValueX, yPosition, 9, 'normal', 'right', theme.text)
    yPosition += 10
    ...
    addText(`$${total.value.toFixed(2)}`, totalsValueX, yPosition, 12, 'bold', 'right', theme.primary)
    ...
    if (paymentQRCode.value) {
      ...
      try {
        addText('Payment QR Code', margin, yPosition, 10, 'bold', 'left', theme.primary)
        yPosition += 8
        ...
        addText(instructionText, margin + 35, yPosition + 8, 9, 'normal', 'left', theme.text)
        addText(`Amount: $${total.value.toFixed(2)}`, margin + 35, yPosition + 15, 9, 'normal', 'left', theme.text)
        addText(`Invoice: ${invoice.value.number}`, margin + 35, yPosition + 22, 9, 'normal', 'left', theme.text)
    ...
    if (invoice.value.payment.instructions || invoice.value.payment.bankDetails) {
      ...
      addText('Payment Information', margin, yPosition, 10, 'bold', 'left', theme.primary)
      yPosition += 8
      ...
      if (invoice.value.payment.instructions) {
        ...
        lines.forEach(line => {
          if (line.trim()) {
            addText(line.trim(), margin, yPosition, 9, 'normal', 'left', theme.text)
            yPosition += 5
          }
        })
        yPosition += 3
      ...
      if (invoice.value.payment.bankDetails) {
        addText('Bank Details:', margin, yPosition, 9, 'bold', 'left', theme.text)
        yPosition += 5
...
watch([
  ...
  if (invoice.value.payment.paypalEmail || invoice.value.payment.cryptoAddress || invoice.value.payment.bankDetails) {
    generatePaymentQR()
  } else {
    paymentQRCode.value = null
  }
...
onMounted(() => {
  ...
  if (savedData) {
    ...
    if (savedData.payment && (savedData.payment.paypalEmail || savedData.payment.cryptoAddress || savedData.payment.bankDetails)) {
      generatePaymentQR()
    }
  }
})
